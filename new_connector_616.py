    ...
    def _transfer_kv_cache(self, req_meta: dict[str, Any]) -> None:
        ...
        for inner_offset, remote_handshake_port in enumerate(remote_handshake_ports):
            ...
            for group_idx, group_remote_block_ids in enumerate(remote_block_ids):
                ...
                kernel_remote_block_ids = [
                    bid * remote_scale + offset
                    for bid in group_remote_block_ids
                    for offset in range(remote_scale)
                ]
                ...
                kernel_cpu_block_ids = [
                    block_map[group_remote_block_ids[i // remote_scale]] * local_scale + i % local_scale
                    for i in range(len(kernel_remote_block_ids))
                ]
                num_kernel_blocks = len(kernel_remote_block_ids)
                kernel_remote_block_ids = kernel_remote_block_ids[:num_kernel_blocks]
                kernel_cpu_block_ids = kernel_cpu_block_ids[:num_kernel_blocks]
...
class KVCacheRecvingThread(threading.Thread):
    ...
    def _transfer_kv_cache_h2d(self, req_meta: dict[str, Any]) -> None:
        ...
        for group_pull in group_pulls:
            ...
            kernel_remote_block_ids = expand_block_ids(remote_group_block_ids, local_scale)
            num_computed_tokens = req_meta.get("num_computed_tokens", 0)
            remote_kernel_block_size = self.block_size // local_scale
            remote_kernel_token_size = remote_kernel_block_size * self.group_compress_ratios[group_idx]
            remote_start_idx = num_computed_tokens // remote_kernel_token_size
            kernel_remote_block_ids = kernel_remote_block_ids[remote_start_idx:]
            kernel_cpu_block_ids = [
                block_map[remote_group_block_ids[(remote_start_idx + i) // local_scale]] * local_scale + i % local_scale
                for i in range(len(kernel_remote_block_ids))
            ]
...
class MooncakeConnectorScheduler:
    ...
    def _build_start_pull_params(
        ...
        transfer_remote_block_ids = self._get_unmatched_remote_block_ids(
            remote_block_ids,
            prompt_len,
            num_computed_tokens,
        )
        ...
        # Block list is already suffix-only; Hop1 must not skip again via token offset.
        pull_params["num_computed_tokens"] = 0
        return pull_params
    ...
    def _get_unmatched_remote_block_ids(
        self,
        remote_block_ids: BlockIds,
        prompt_len: int,
        num_computed_tokens: int,
    ) -> BlockIds:
        """Return remote blocks that still need to be pulled (prefix-cache miss suffix).

        Mirrors the intent of ``get_num_new_matched_tokens``: locally matched prefix
        blocks are excluded so D2RH only stages and pulls the delta.
        """
        if not remote_block_ids or num_computed_tokens <= 0:
            return self._get_transfer_block_ids(remote_block_ids, prompt_len)

        effective_prompt_len = self._state_prefill_token_count(prompt_len)
        transfer_block_ids = self._get_transfer_block_ids(remote_block_ids, effective_prompt_len)
        unmatched: list[list[int]] = []
        for blocks, group_info in zip(transfer_block_ids, self.group_transfer_info):
            if not blocks:
                unmatched.append(blocks)
                continue
            if group_info.is_state_group:
                # Mamba/state: keep full transferable blocks while prompt suffix remains.
                unmatched.append(blocks if num_computed_tokens < effective_prompt_len else [])
            elif group_info.blocks_per_window > 0:
                window_token_len = group_info.blocks_per_window * group_info.tokens_per_block
                tokens_before_window = max(0, effective_prompt_len - window_token_len)
                if num_computed_tokens <= tokens_before_window:
                    unmatched.append(blocks)
                else:
                    tokens_in_window = num_computed_tokens - tokens_before_window
                    num_prefix = min(len(blocks), tokens_in_window // group_info.tokens_per_block)
                    unmatched.append(blocks[num_prefix:])
            else:
                num_prefix = min(len(blocks), num_computed_tokens // group_info.tokens_per_block)
                unmatched.append(blocks[num_prefix:])
    ...
    def get_num_new_matched_tokens(self, request: "Request", num_computed_tokens: int) -> tuple[int, bool]:
        ...
        if params is not None and params.get("do_remote_prefill"):
            ...
            actual = self._state_prefill_token_count(prompt_len)
            params["num_computed_tokens"] = num_computed_tokens
            count = max(actual - num_computed_tokens, 0)
            if count <= 0:
                return 0, False

            if self.kv_role == "kv_consumer" and request.request_id not in self.all_requests:
                try:
...
class MooncakeConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self.kv_role == "kv_consumer" and cpu_register_ptrs:
