...
class MooncakeAgentMetadata(msgspec.Struct, omit_defaults=True, dict=True):
    ...
    num_blocks: int
    local_ip: str = ""
    kv_caches_base_addr_nested: list[list[int]] | None = None
    block_lens: list[list[int]] | None = None
    block_strides: list[list[int]] | None = None
    kv_group2layeridx: dict[int, Any] | None = None
...
class cpu_kvcache_manager():
    """Unified CPU staging block pool shared across all KV cache tensors."""
    ...
    @staticmethod
    def normalize_grouped_block_ids(
    ...
    @staticmethod
    def is_nested_block_map(block_map: dict[int, Any]) -> bool:
        if not block_map:
            return False
        return isinstance(next(iter(block_map.values())), dict)

    @staticmethod
    def lookup_cpu_block_id(
        block_map: dict[int, Any],
        remote_block_id: int,
        group_idx: int | None = None,
    ) -> int:
        if cpu_kvcache_manager.is_nested_block_map(block_map):
            if group_idx is not None:
                return block_map[group_idx][remote_block_id]
            for group_map in block_map.values():
                if remote_block_id in group_map:
                    return group_map[remote_block_id]
            raise KeyError(
                f"remote block {remote_block_id} not found in nested block map"
            )
        return block_map[remote_block_id]

    @staticmethod
    def flatten_block_map(
        block_map: dict[int, Any],
    ) -> tuple[list[int], list[int]]:
        if cpu_kvcache_manager.is_nested_block_map(block_map):
            remote_block_ids: list[int] = []
            cpu_block_ids: list[int] = []
            for group_idx in sorted(block_map):
                for remote_id, cpu_id in block_map[group_idx].items():
                    remote_block_ids.append(remote_id)
                    cpu_block_ids.append(cpu_id)
            return remote_block_ids, cpu_block_ids
        remote_block_ids = list(block_map.keys())
        cpu_block_ids = list(block_map.values())
        return remote_block_ids, cpu_block_ids

    def alloc_blocks(self, req_need_blocks: int) -> list[int] | None:
        ...
        if req_need_blocks > len(self.free_queue):
            ...
            return None
        local_block_ids = [
            self.free_queue.popleft() for _ in range(req_need_blocks)
        ]
        self.used_set.update(local_block_ids)
        logger.info(f"Alloc blocks{local_block_ids}")
        return local_block_ids

    def alloc_blocks_for_groups(
        self,
        grouped_remote_block_ids: tuple[list[int], ...] | list[int] | list[list[int]],
    ) -> dict[int, dict[int, int]] | None:
        grouped_remote = self.normalize_grouped_block_ids(grouped_remote_block_ids)
        total_needed = sum(len(group) for group in grouped_remote)
        if total_needed > len(self.free_queue):
            logger.info(
                "Dont have enough blocks for groups %s total=%d free=%d",
                grouped_remote,
                total_needed,
                len(self.free_queue),
            )
        ...
        block_map: dict[int, dict[int, int]] = {}
        allocated_cpu_ids: list[int] = []
        try:
            for group_idx, remote_block_ids in enumerate(grouped_remote):
                if not remote_block_ids:
                    block_map[group_idx] = {}
                    continue
                cpu_block_ids = [
                    self.free_queue.popleft() for _ in range(len(remote_block_ids))
                ]
                allocated_cpu_ids.extend(cpu_block_ids)
                block_map[group_idx] = dict(zip(remote_block_ids, cpu_block_ids))
            self.used_set.update(allocated_cpu_ids)
            logger.info(
                "Alloc blocks for groups remote=%s map=%s",
                grouped_remote,
                block_map,
            )
            return block_map
        except IndexError:
            self.free_queue.extend(allocated_cpu_ids)
            self.used_set.difference_update(allocated_cpu_ids)
            logger.info(
                "Rollback partial group allocation remote=%s allocated=%s",
                grouped_remote,
                allocated_cpu_ids,
            )
            return None

    def free_blocks(self, req_block_ids: list[list[int]]):
        flat_block_ids = [block_id for sublist in req_block_ids for block_id in sublist]
        self._return_blocks(flat_block_ids)

    def free_blocks_from_map(self, block_map: dict[int, Any]) -> None:
        if self.is_nested_block_map(block_map):
            cpu_block_ids = [
                cpu_id
                for group_map in block_map.values()
                for cpu_id in group_map.values()
            ]
        else:
            cpu_block_ids = list(block_map.values())
        self._return_blocks(cpu_block_ids)

    def _return_blocks(self, block_ids: list[int]) -> None:
        if not block_ids:
            return
        unique_block_ids = set(block_ids)
        self.free_queue.extend(unique_block_ids)
        self.used_set.difference_update(unique_block_ids)
...
class D2RHThread(threading.Thread):
    def __init__(
            ...
            cache_flat_meta: list[CacheFlatMeta] | None = None,
            group_flat_indices: dict[int, list[int]] | None = None,
            block_len_per_addr: list[list[int]] | None = None,
            block_stride_per_addr: list[list[int]] | None = None,
            cpu_kv_caches_base_addr_nested: list[list[int]] | None = None,
            ):
        ...
        self.remote_kv_caches_base_addr_nested: dict[str, dict[int, list[list[int]]]] = SizedDict()
        self.remote_block_lens: dict[str, dict[int, list[list[int]]]] = SizedDict()
        self.remote_block_strides: dict[str, dict[int, list[list[int]]]] = SizedDict()
        ...
        self.cache_flat_meta = cache_flat_meta or []
        self.group_flat_indices = group_flat_indices or {}
        self.block_len_per_addr = block_len_per_addr or []
        self.block_stride_per_addr = block_stride_per_addr or []
        self.cpu_kv_caches_base_addr_nested = cpu_kv_caches_base_addr_nested or []
                ...
                if msg[0] == START_PULL:
                    ...
                    try:
                        ...
                        if not remote_handshake_ports:
                            ...
                            remote_handshake_port = params.get(
                                'remote_handshake_port', remote_port_base
                            )
                            remote_handshake_ports = [remote_handshake_port]
                        else:
                            remote_port_base = params['remote_port']
                        block_map = self.cpu_kvcache_manager.alloc_blocks_for_groups(
                            remote_block_ids
                        )
                        if block_map is not None:
                            ...
                            flat_remote, flat_cpu = (
                                self.cpu_kvcache_manager.flatten_block_map(block_map)
                            )
                            logger.info(
                                "[===] CPU staging blocks allocated: remote=%s cpu=%s map=%s",
                                flat_remote,
                                flat_cpu,
                                block_map,
                            )
                            self.add_request(
                                ...
                                grouped_remote_block_ids=flat_remote,
                                grouped_local_block_ids=flat_cpu,
                            )
                        else:
                            grouped_remote = (
                                self.cpu_kvcache_manager.normalize_grouped_block_ids(
                                    remote_block_ids
                                )
                            )
                            logger.warning(
                                ...
                                request_id,
                                sum(len(group) for group in grouped_remote),
                            )
    ...
    def _transfer_kv_cache(self, req_meta: dict[str, Any]):
        ...
        remote_handshake_ports = req_meta.get("remote_handshake_ports") or [
            req_meta.get("remote_port_base", 0)
        ]
        ...
        block_map = (
            self.remote_local_block_map.get(remote_request_id)
            or self.remote_local_block_map.get(request_id)
        )
        use_hybrid_transfer = (
            self.use_hybrid
            and block_map is not None
            and cpu_kvcache_manager.is_nested_block_map(block_map)
        )
        ...
        for inner_offset, remote_handshake_port in enumerate(remote_handshake_ports):
            ...
            if use_hybrid_transfer:
                remote_nested = self.remote_kv_caches_base_addr_nested[port_engine_id][
                    remote_handshake_port
                ]
                remote_block_lens = self.remote_block_lens[port_engine_id][remote_handshake_port]
                remote_block_strides = self.remote_block_strides[port_engine_id][remote_handshake_port]
                local_list, peer_list, length_list = build_group_transfer_batch(
                    block_map=block_map,
                    group_flat_indices=self.group_flat_indices,
                    cache_flat_meta=self.cache_flat_meta,
                    local_ptrs=self.cpu_ptrs,
                    remote_ptrs_nested=remote_nested,
                    local_block_strides=self.block_stride_per_addr,
                    remote_block_lens=remote_block_lens,
                    remote_block_strides=remote_block_strides,
                ...
                for layer_idx, remote_layer_base in enumerate(remote_kv_caches_base_addrs):
                    ...
                    for remote_block_id, cpu_block_id in zip(grouped_remote_block_ids, grouped_cpu_block_ids):
                        local_list.append(
                            cpu_layer_base + cpu_block_id[0] * block_len + inner_offset * inner_block_len
                        )
    ...
    def _get_remote_metadata(self, remote_host: str, remote_handshake_port: int) -> None:
        ...
        try:
            ...
            if agent_meta.kv_caches_base_addr_nested is not None:
                self.remote_kv_caches_base_addr_nested[engine_id][remote_handshake_port] = (
                    agent_meta.kv_caches_base_addr_nested
                )
            if agent_meta.block_lens is not None:
                self.remote_block_lens[engine_id][remote_handshake_port] = agent_meta.block_lens
            if agent_meta.block_strides is not None:
                self.remote_block_strides[engine_id][remote_handshake_port] = agent_meta.block_strides
...
class KVCacheRecvingThread(threading.Thread):
    def __init__(
        ...
        self.block_len_per_addr = block_len_per_addr or []
        self.block_stride_per_addr = block_stride_per_addr or []
        self.kv_caches_base_addr_nested = kv_caches_base_addr_nested or []
        self.cpu_kv_caches_base_addr_nested = cpu_kv_caches_base_addr_nested or []
    ...
    def _transfer_kv_cache(self, req_meta: dict[str, Any]):
        ...
        use_hybrid_transfer = (
            self.use_hybrid
            and cpu_kvcache_manager.is_nested_block_map(cpu_block_map)
            and self.cache_flat_meta
        )
        if use_hybrid_transfer:
            layer_filter = set(range(first_layer_index, end_layer_index))
            flat_npu_ptrs = self.kv_caches_base_addr[self.local_engine_id][self.local_handshake_port]
            remote_to_group: dict[int, int] = {}
            for group_id, group_map in cpu_block_map.items():
                for remote_id in group_map:
                    remote_to_group[remote_id] = group_id
            for meta in self.cache_flat_meta:
                if meta.layer_idx not in layer_filter:
                    continue
                block_len = meta.block_len
                block_stride = meta.block_stride
                if self.block_stride_per_addr:
                    block_stride = self.block_stride_per_addr[meta.layer_idx][meta.cache_idx]
                inner_block_len = block_len // tp_num_need_pulls
                npu_base = flat_npu_ptrs[meta.flat_idx]
                cpu_base = self.cpu_ptrs[meta.flat_idx]
                for remote_group, npu_group in zip(grouped_remote_block_ids, grouped_local_block_ids):
                    remote_id = remote_group[0]
                    if remote_to_group.get(remote_id) != meta.group_id:
                        continue
                    cpu_block_id = cpu_block_map[meta.group_id][remote_id]
                    npu_block_id = npu_group[0]
                    local_list.append(
                        npu_base + npu_block_id * block_stride + inner_offset * inner_block_len
                    )
                    peer_list.append(
                        cpu_base + cpu_block_id * block_stride + inner_offset * inner_block_len
                    )
                    length_list.append(inner_block_len * len(npu_group))
        else:
            block_length = len(self.block_len)
            for k, (npu_layer_base, cpu_layer_base) in enumerate(
                ...
                for remote_block_id, npu_block_id in zip(grouped_remote_block_ids, grouped_local_block_ids):
                    cpu_block_id = self.cpu_kvcache_manager.lookup_cpu_block_id(
                        cpu_block_map,
                        remote_block_id[0],
                        req_meta.get("group_idx"),
                    )
                    local_list.append(
        ...
        if req_meta.get("all_task_done") and self.cpu_kvcache_manager is not None:
            self.cpu_kvcache_manager.free_blocks_from_map(cpu_block_map)
...
class MooncakeConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str, kv_cache_config: KVCacheConfig):
        ...
        self.max_device_id = (
            ...
            vllm_config.parallel_config.data_parallel_size * self.pcp_size *
        ...
        self.group_transfer_info = [
            self._get_group_transfer_info(group) for group in kv_cache_config.kv_cache_groups
        ]
    ...
    def _get_group_transfer_info(self, group: Any) -> GroupTransferInfo:
        ...
        if not isinstance(block_size, int):
            block_size = getattr(first_spec, "block_size", None)
        if not isinstance(block_size, int):
    ...
    def _state_prefill_token_count(self, num_prompt_tokens: int) -> int:
        """D-side: N-1 for hybrid models when decoder recomputes the last token."""
        if self.need_truncate and num_prompt_tokens > 1:
            return num_prompt_tokens - 1
        return num_prompt_tokens

    def _truncate_request_for_prefill(self, request: "Request") -> None:
        """P-side: drop the last prompt token once for hybrid remote decode."""
        params = request.kv_transfer_params
        if (
            params is not None
            and not params.get("_p_side_truncated")
            and getattr(request, "num_prompt_tokens", len(request.prompt_token_ids or [])) > 1
        ):
                    ...
                    try:
                        ...
                        for decode_tp_rank in range(self._decode_tp_size):
                            ...
                            pull_params = self._build_start_pull_params(
                                request.request_id, params, decode_tp_rank
                            )
                            resp = self._send_start_pull(
                                request.request_id, pull_params, d2rh_port
                            )
                            logger.info(
                                "START_PULL request=%s decode_tp_rank=%d d2rh_port=%d "
                                "remote_handshake_ports=%s resp=%s",
                                request.request_id,
                                decode_tp_rank,
                                d2rh_port,
    ...
    def request_finished(
        ...
        if delay_free_blocks:
            logger.info("Delaying free of %d blocks for request %s",
                        sum(computed_block_lens), request.request_id)
...
class MooncakeConnectorWorker:
    ...
    def __init__(
        ...
        self.kv_cache_config = kv_cache_config
        self.total_layers = vllm_config.model_config.hf_text_config.num_hidden_layers
        ...
        self.cache_flat_meta: list[CacheFlatMeta] = []
        self.group_flat_indices: dict[int, list[int]] = {}
        self.block_len_per_addr: list[list[int]] = []
        self.block_stride_per_addr: list[list[int]] = []
        self.kv_caches_base_addr_nested: list[list[int]] = []
        self.cpu_kv_caches_base_addr_nested: list[list[int]] = []
    ...
    def _build_kv_group2layeridx(self) -> dict[int, tuple[dict[str, Any], list[int]]]:
        ...
        num_attn_module = (
            2 if self.vllm_config.model_config.hf_text_config.model_type == "longcat_flash" else 1
        )
        ...
        for group_id, group_spec in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            for layer_name in group_spec.layer_names:
                if "mtp" in layer_name:
                    ...
                    if assigned_indices and (
                        layer_idx < min(assigned_indices) or layer_idx in assigned_indices
                    ):
                        layer_idx = next_mtp_layer_idx
                        next_mtp_layer_idx += 1
            ...
            kv_group2layeridx[group_id] = (
                self._serialize_kv_group_spec(group_spec),
                layer_indices,
            )
    ...
    def _build_hybrid_layout(
        self,
        kv_caches: dict[str, torch.Tensor],
    ) -> tuple[list[int], dict[str, int]]:
        """Build nested KV layout, flat index tables, and flat NPU ptr list."""
        assert self.kv_cache_config is not None
        ...
        self.kv_caches_base_addr_nested = [[] for _ in range(metadata_layers)]
        self.block_len_per_addr = [[] for _ in range(metadata_layers)]
        self.block_stride_per_addr = [[] for _ in range(metadata_layers)]
        ...
        for layer_name, kv_cache_tuple in kv_caches.items():
            ...
            for single_kv_cache in self._as_kv_cache_tuple(kv_cache_tuple):
                ...
                self.kv_caches_base_addr_nested[layer_idx].append(single_kv_cache.data_ptr())

        cache_flat_meta: list[CacheFlatMeta] = []
        flat_npu_ptrs: list[int] = []
        for layer_idx in range(metadata_layers):
            for cache_idx, base_addr in enumerate(self.kv_caches_base_addr_nested[layer_idx]):
                block_len = self.block_len_per_addr[layer_idx][cache_idx]
                block_stride = self.block_stride_per_addr[layer_idx][cache_idx]
                group_id = layer_to_group[layer_idx]
                cache_flat_meta.append(
                    CacheFlatMeta(
                        group_id=group_id,
                        layer_idx=layer_idx,
                        cache_idx=cache_idx,
                        ...
                        flat_idx=len(flat_npu_ptrs),
                    )
                )
                flat_npu_ptrs.append(base_addr)

        self.cache_flat_meta = cache_flat_meta
        self.group_flat_indices = build_group_flat_indices(cache_flat_meta)
        return flat_npu_ptrs, layer_name_to_idx

    def _register_kv_caches_hybrid(self, kv_caches: dict[str, torch.Tensor]):
        ...
        logger.info("num_blocks: %s use_hybrid=%s", self.num_blocks, self.use_hybrid)
        self.kv_caches = kv_caches
        flat_npu_ptrs, layer_name_to_idx = self._build_hybrid_layout(kv_caches)
        self.block_len = [meta.block_len for meta in self.cache_flat_meta]

        ptrs: list[int] = []
        lengths: list[int] = []
        for ptr, meta in zip(flat_npu_ptrs, self.cache_flat_meta):
            ptrs.append(ptr)
            lengths.append(self.num_blocks * meta.block_len)
        ...
        metadata_layers = len(self.kv_caches_base_addr_nested)

        if self.kv_role == "kv_consumer":
            cpu_kvcache_manager_instance = cpu_kvcache_manager(self.num_blocks)
            self.cpu_kv_caches_base_addr_nested = [[] for _ in range(metadata_layers)]
            layer_idx_to_name = {idx: name for name, idx in layer_name_to_idx.items()}
            for layer_idx in range(metadata_layers):
                layer_name = layer_idx_to_name[layer_idx]
                kv_cache_tuple = kv_caches[layer_name]
                for cache_idx, npu_cache in enumerate(self._as_kv_cache_tuple(kv_cache_tuple)):
                    cpu_cache = torch.empty(
                        npu_cache.shape,
                        dtype=npu_cache.dtype,
                        device="cpu",
                        pin_memory=True,
                    )
                    self.cpu_caches_hold.append(cpu_cache)
                    cpu_ptr = cpu_cache.data_ptr()
                    self.cpu_kv_caches_base_addr_nested[layer_idx].append(cpu_ptr)
                    cpu_kv_caches_base_addr.append(cpu_ptr)
                    block_len = self.block_len_per_addr[layer_idx][cache_idx]
                    ptrs.append(cpu_ptr)
                    lengths.append(self.num_blocks * block_len)

            d2rh_port = get_d2rh_zmq_port(
                self.vllm_config, self.tp_rank, self.pp_rank, self.pcp_rank
            )
            ...
            self.d2rh_thread = D2RHThread(
                ...
                cpu_kv_caches_base_addr_nested=self.cpu_kv_caches_base_addr_nested,
            )
        ...
        metadata = MooncakeAgentMetadata(
            ...
            kv_caches_base_addr=flat_npu_ptrs,
            num_blocks=self.num_blocks,
            local_ip=get_ip(),
            kv_caches_base_addr_nested=self.kv_caches_base_addr_nested,
            block_lens=self.block_len_per_addr,
            block_strides=self.block_stride_per_addr,
            kv_group2layeridx=self.kv_group2layeridx if self.use_hybrid else None,
        )
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self.kv_cache_config is not None:
            self._register_kv_caches_hybrid(kv_caches)
            return

        self._register_kv_caches_legacy(kv_caches)

    def _register_kv_caches_legacy(self, kv_caches: dict[str, torch.Tensor]):
...
def build_group_flat_indices(cache_flat_meta: list[CacheFlatMeta]) -> dict[int, list[int]]:
    group_flat: dict[int, list[int]] = defaultdict(list)
    for meta in cache_flat_meta:
        group_flat[meta.group_id].append(meta.flat_idx)
    return dict(group_flat)


def build_group_transfer_batch(
    block_map: dict[int, dict[int, int]],
    group_flat_indices: dict[int, list[int]],
    cache_flat_meta: list[CacheFlatMeta],
    local_ptrs: list[int],
    remote_ptrs_nested: list[list[int]],
    local_block_strides: list[list[int]] | None,
    remote_block_lens: list[list[int]] | None,
    remote_block_strides: list[list[int]] | None,
    tp_num_need_pulls: int,
    inner_offset: int,
    layer_idx_filter: set[int] | None = None,
) -> tuple[list[int], list[int], list[int]]:
    """Build Mooncake batch transfer lists: group -> layers -> blocks."""
    local_list: list[int] = []
    peer_list: list[int] = []
    length_list: list[int] = []
    for group_id in sorted(block_map):
        remote_to_local = block_map[group_id]
        if not remote_to_local:
            continue
        remote_ids = list(remote_to_local.keys())
        local_ids = [remote_to_local[remote_id] for remote_id in remote_ids]
        if tp_num_need_pulls == 1:
            grouped_remote, grouped_local = group_concurrent_contiguous(remote_ids, local_ids)
        else:
            grouped_remote = [[remote_id] for remote_id in remote_ids]
            grouped_local = [[local_id] for local_id in local_ids]

        for flat_idx in group_flat_indices.get(group_id, []):
            meta = cache_flat_meta[flat_idx]
            if layer_idx_filter is not None and meta.layer_idx not in layer_idx_filter:
                continue
            block_len = meta.block_len
            block_stride = meta.block_stride
            if local_block_strides:
                block_stride = local_block_strides[meta.layer_idx][meta.cache_idx]
            remote_stride = block_stride
            remote_bl = block_len
            if remote_block_strides:
                remote_stride = remote_block_strides[meta.layer_idx][meta.cache_idx]
            if remote_block_lens:
                remote_bl = remote_block_lens[meta.layer_idx][meta.cache_idx]
            inner_block_len = block_len // tp_num_need_pulls
            inner_remote_block_len = remote_bl // tp_num_need_pulls
            local_base = local_ptrs[flat_idx]
            remote_base = remote_ptrs_nested[meta.layer_idx][meta.cache_idx]
            for remote_group, local_group in zip(grouped_remote, grouped_local):
                local_list.append(
                    local_base + local_group[0] * block_stride + inner_offset * inner_block_len
                )
                peer_list.append(
                    remote_base
                    + remote_group[0] * remote_stride
                    + inner_offset * inner_remote_block_len
                )
                length_list.append(inner_block_len * len(local_group))
    return local_list, peer_list, length_list
...
def get_remote_ranks_for_req(
    ...
    if prefill_tp_size == decode_tp_size:
        return [
            [tp + pp * prefill_tp_size for pp in range(prefill_pp_size)]
    ...
    rand_group_index = rand.sample(
        range(num_groups), max(decode_tp_size // num_kv_head, 1)
    )
    all_results = [
        get_remote_tp_ranks(
            ori_data[pp_index],
