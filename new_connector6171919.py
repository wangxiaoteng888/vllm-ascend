...
# Bind/listen on the node data-plane IP; Scheduler connects to the same get_ip().
D2RH_START_PULL_ZMQ_TIMEOUT = 15.0
# Periodic log interval while blocked in staging/hop1 waits (seconds).
D2RH_TRACE_WAIT_LOG_INTERVAL = 10.0


def d2rh_trace(stage: str, request_id: str = "", **fields: Any) -> None:
    """Single-line D2RH pipeline checkpoint; grep ``[D2RH-TRACE]`` to follow a request."""
    parts = [f"[D2RH-TRACE] stage={stage}"]
    if request_id:
...
class HostListeningThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        logger.info(
            "[D2RH-DEBUG] scheduler READY listener starting pid=%s path=%s decode_tp_size=%d",
            os.getpid(),
            path,
            self.decode_tp_size,
        )
        try:
            with zmq_ctx(zmq.ROUTER, path) as sock:  # type: ignore
                logger.info(
                    "[D2RH-DEBUG] scheduler READY listener bound pid=%s path=%s",
                    os.getpid(),
                    path,
                )
...
class D2RHThread(threading.Thread):
    ...
    def __init__(
        ...
        staging_hop1_ready: dict[str, threading.Event] | None,
        ...
        self.staging_hop1_ready = staging_hop1_ready if staging_hop1_ready is not None else {}
    ...
    def run(self) -> None:
        ...
        logger.info(
            "[D2RH-DEBUG] Starting D2RH listener tp_rank=%s path=%s",
            self.tp_rank,
            path,
        )
        ...
        def zmq_listener_worker() -> None:
            try:
                with zmq_ctx(zmq.ROUTER, path) as sock:  # type: ignore
                    logger.info(
                        "[D2RH-DEBUG] D2RH ZMQ ROUTER bound tp_rank=%s path=%s pid=%s tid=%s",
                        self.tp_rank,
                        path,
                        os.getpid(),
                ...
                logger.error(
                    "[D2RH-DEBUG] D2RH ZMQ listener crashed tp_rank=%s path=%s pid=%s err=%s",
                    self.tp_rank,
                    path,
                    os.getpid(),
                    e,
        ...
        while True:
            ...
            hop1_req_id = req_meta.get("request_id", "")
            d2rh_trace(
                "d2rh.hop1.dequeue",
                request_id=hop1_req_id,
                tp_rank=self.tp_rank,
                pid=os.getpid(),
                remote_request_id=req_meta.get("remote_request_id", hop1_req_id),
            )
            ...
            try:
                ...
                d2rh_trace(
                    ...
                    request_id=hop1_req_id,
                    tp_rank=self.tp_rank,
                    elapsed_s=f"{time.time() - hop1_start:.3f}",
                )
            finally:
                self.request_queue.task_done()

    def _reply_start_pull(
        self,
        ...
        while True:
            try:
                ...
                logger.info(
                    "[D2RH-DEBUG] START_PULL reply tp_rank=%s port=%s pid=%s request_id=%s "
                    "ack=%s reason=%s",
                    self.tp_rank,
                    self.d2rh_handshake_port,
                    os.getpid(),
                    request_id,
                    pull_ack.decode("utf-8", errors="replace"),
                    reason,
                )
                return
            except zmq.Again:  # type: ignore
    ...
    def _run_start_pull_loop(self, sock: zmq.Socket) -> None:  # type: ignore
        ...
        while True:
            ...
            try:
                frames = sock.recv_multipart()
                logger.info(
                    "[D2RH-DEBUG] ZMQ recv tp_rank=%s port=%s pid=%s num_frames=%d frame_lens=%s",
                    self.tp_rank,
                    self.d2rh_handshake_port,
                    os.getpid(),
                    len(frames),
                ...
                if len(frames) < 2:
                    logger.warning(
                        "[D2RH-DEBUG] START_PULL drop short frames tp_rank=%s port=%s n=%d",
                        self.tp_rank,
                        self.d2rh_handshake_port,
                        len(frames),
                    )
                ...
                if len(payload) != 1:
                    logger.warning(
                        "[D2RH-DEBUG] START_PULL bad payload tp_rank=%s port=%s payload_parts=%d",
                        self.tp_rank,
                        self.d2rh_handshake_port,
                        len(payload),
                    )
                    self._reply_start_pull(sock, identity, STAGING_FULL, "", "bad_payload")
                    continue
                try:
                    msg = decoder.decode(payload[0])
                except Exception as decode_err:
                    logger.error(
                        "[D2RH-DEBUG] START_PULL decode failed tp_rank=%s port=%s err=%s payload_prefix=%r",
                        self.tp_rank,
                        self.d2rh_handshake_port,
                        decode_err,
                        payload[0][:64],
                    )
                    self._reply_start_pull(sock, identity, STAGING_FULL, "", "decode_failed")
                    continue
                if msg[0] != START_PULL:
                    logger.error(
                        "[D2RH-DEBUG] START_PULL unexpected msg tp_rank=%s port=%s msg_type=%r",
                        self.tp_rank,
                        self.d2rh_handshake_port,
                        msg[0],
                    )
                    self._reply_start_pull(sock, identity, STAGING_FULL, "", "unexpected_msg")
                    continue

                request_id = msg[1]
                params = msg[2]
                d2rh_trace(
                    "d2rh.zmq_start_pull.received",
                    request_id=request_id,
                    tp_rank=self.tp_rank,
                    pid=os.getpid(),
                    port=self.d2rh_handshake_port,
                ...
                logger.info(
                    ...
                    self.d2rh_handshake_port,
                    request_id,
                    params.get("remote_request_id", request_id),
                    params.get("remote_block_ids"),
                )
                pull_ack = self._handle_start_pull_params(request_id, params, source="zmq")
                self._reply_start_pull(sock, identity, pull_ack, request_id, "handled")
            except Exception as e:
                logger.error(
                    "[D2RH-DEBUG] START_PULL loop exception tp_rank=%s port=%s pid=%s request_id=%s err=%s",
                    self.tp_rank,
                    self.d2rh_handshake_port,
                    os.getpid(),
                    request_id,
                ...
                if identity is not None:
                    self._reply_start_pull(sock, identity, STAGING_FULL, request_id, "loop_exception")

    def _register_hop1_ready(self, request_id: str, remote_request_id: str) -> threading.Event:
        evt = threading.Event()
        self.staging_hop1_ready[request_id] = evt
        if remote_request_id != request_id:
            self.staging_hop1_ready[remote_request_id] = evt
        return evt

    def _mark_hop1_ready(self, request_id: str, remote_request_id: str) -> None:
        evt = self.staging_hop1_ready.get(request_id) or self.staging_hop1_ready.get(remote_request_id)
        if evt is not None:
            evt.set()

    def prepare_staging_local(self, request_id: str, params: dict[str, Any]) -> bool:
        """Allocate CPU staging and queue Hop1 in-process (no EngineCore ZMQ)."""
        d2rh_trace(
            "worker.staging.ensure.enter",
            request_id=request_id,
            tp_rank=self.tp_rank,
            pid=os.getpid(),
            remote_request_id=params.get("remote_request_id", request_id),
        )
        pull_ack = self._handle_start_pull_params(request_id, params, source="worker_local")
        ok = pull_ack == b"ACK"
        d2rh_trace(
            "worker.staging.ensure.exit",
            request_id=request_id,
            tp_rank=self.tp_rank,
            ok=ok,
            resp=pull_ack.decode("utf-8", errors="replace"),
            staging_keys=sorted(self.staging_allocations.keys()),
        )
        return ok

    def _handle_start_pull_params(
        self,
        request_id: str,
        params: dict[str, Any],
        source: str = "zmq",
        ...
        with self._staging_lock:
            remote_request_id = params.get("remote_request_id", request_id)
            if (
                remote_request_id in self.staging_allocations
                or request_id in self.staging_allocations
            ):
                logger.info(
                    "[D2RH-DEBUG] staging already present tp_rank=%s source=%s request_id=%s keys=%s",
                    self.tp_rank,
                    source,
                    request_id,
            ...
            try:
                logger.info(
                    "[D2RH-DEBUG] staging prepare begin tp_rank=%s source=%s pid=%s request_id=%s "
                    "remote_request_id=%s remote_block_ids=%s",
                    self.tp_rank,
                    source,
                    os.getpid(),
                ...
                if HostStagingManager.staging_demand(remote_block_ids) == 0:
                    ...
                    hop1_evt = self._register_hop1_ready(request_id, remote_request_id)
                    hop1_evt.set()
                    logger.info(
                        "[D2RH-DEBUG] staging allocated (empty) tp_rank=%s source=%s request_id=%s "
                        "staging_keys=%s",
                        self.tp_rank,
                        source,
                        request_id,
                    ...
                    if staging is None:
                        ...
                        logger.info(
                            "[D2RH-DEBUG] START_PULL STAGING_FULL tp_rank=%s source=%s request_id=%s",
                            self.tp_rank,
                            source,
                            request_id,
                        )
                        ...
                        self._register_hop1_ready(request_id, remote_request_id)
                        logger.info(
                            "[D2RH-DEBUG] staging allocated tp_rank=%s source=%s request_id=%s "
                            "blocks=%s staging_keys=%s",
                            self.tp_rank,
                            source,
                            request_id,
                ...
                logger.exception(
                    "Failed to prepare staging tp_rank=%s source=%s request_id=%s",
                    self.tp_rank,
                    source,
                    request_id,
                )
    ...
    def _transfer_kv_cache(self, req_meta: dict[str, Any]) -> None:
        ...
        hop1_begin = time.time()
        d2rh_trace(
            "d2rh.hop1.begin",
            request_id=request_id,
            tp_rank=self.tp_rank,
            remote_request_id=remote_request_id,
            remote_host=req_meta.get("remote_host"),
            remote_ports=req_meta.get("remote_handshake_ports"),
        )
        logger.info(
        ...
        if transfer_ok:
            ...
            d2rh_trace(
                ...
                tp_rank=self.tp_rank,
                remote_request_id=remote_request_id,
                elapsed_s=f"{time.time() - hop1_begin:.3f}",
            )
            self._mark_hop1_ready(request_id, remote_request_id)
...
class KVCacheRecvingThread(threading.Thread):
    def __init__(
        ...
        staging_hop1_ready: dict[str, threading.Event] | None = None,
    ):
    ...
    def _handle_request(self, req_meta: dict[str, Any]):
        ...
        num_local_blocks = sum(len(group_ids) for group_ids in req_meta.get("local_block_ids", tuple()))
        ...
        d2rh_trace(
            ...
            all_task_done=all_task_done,
            use_cpu_staging=self.use_cpu_staging,
            queue_size=self.request_queue.qsize(),
        )
        ...
        try:
            ...
            reported_done = all_task_done and not transfer_failed and (not self.use_cpu_staging or num_local_blocks > 0)
            d2rh_trace(
                "h2d.handle.exit",
                request_id=request_id,
                tp_rank=self.tp_rank,
                ...
                elapsed_s=f"{time.time() - h2d_begin:.3f}",
            )
            # Do not report finished_recving when D2RH/H2D failed or was a no-op.
            if reported_done:
                self.task_tracker.update_done_task_count(request_id)
    ...
    def _wait_for_staging(self, request_id: str, remote_request_id: str) -> StagingAllocation:
        ...
        while time.time() < deadline:
            ...
            if staging is not None:
                ...
                self._wait_for_hop1_ready(request_id, remote_request_id, deadline)
                return staging
            now = time.time()
            if now - last_log_time >= D2RH_TRACE_WAIT_LOG_INTERVAL:
                d2rh_trace(
                    "h2d.wait_staging.blocked",
                    request_id=request_id,
                    tp_rank=self.tp_rank,
                    remote_request_id=remote_request_id,
                    waited_s=f"{now - wait_start:.1f}",
                    staging_keys=sorted(self.staging_allocations.keys()),
                    hop1_keys=sorted(self.staging_hop1_ready.keys()),
                )
                logger.warning(
                    "[D2RH-DEBUG] H2D still waiting staging tp_rank=%s request_id=%s remote_request_id=%s "
                    "waited_s=%.1f staging_keys=%s (EngineCore START_PULL may not have reached this worker)",
                    self.tp_rank,
                    request_id,
                    remote_request_id,
                    now - wait_start,
            ...
            time.sleep(0.01)
        d2rh_trace(
            "h2d.wait_staging.timeout",
            request_id=request_id,
            tp_rank=self.tp_rank,
            remote_request_id=remote_request_id,
            waited_s=f"{time.time() - wait_start:.1f}",
            staging_keys=sorted(self.staging_allocations.keys()),
        )
        logger.error(
    ...
    def _wait_for_hop1_ready(self, request_id: str, remote_request_id: str, deadline: float) -> None:
        if not self.staging_hop1_ready:
            return
        hop1_start = time.time()
        last_log_time = hop1_start
        d2rh_trace(
            "h2d.wait_hop1.enter",
            request_id=request_id,
            tp_rank=self.tp_rank,
        ...
        while time.time() < deadline:
            ...
            if evt is not None:
                ...
                if remaining <= 0 or evt.wait(timeout=remaining):
                    d2rh_trace(
                        "h2d.wait_hop1.done",
                        request_id=request_id,
                        tp_rank=self.tp_rank,
                        remote_request_id=remote_request_id,
                        waited_s=f"{time.time() - hop1_start:.3f}",
                    )
                    logger.info(
                        "[D2RH-DEBUG] H2D wait hop1 done tp_rank=%s request_id=%s remote_request_id=%s "
                        "waited_s=%.3f",
                        self.tp_rank,
                        request_id,
                        remote_request_id,
                        time.time() - hop1_start,
                    )
                    return
                d2rh_trace(
                    "h2d.wait_hop1.timeout",
                    request_id=request_id,
                    tp_rank=self.tp_rank,
                    remote_request_id=remote_request_id,
                    waited_s=f"{time.time() - hop1_start:.3f}",
                ...
                raise RuntimeError(
                    f"Hop1 transfer not complete for request {remote_request_id} before H2D deadline."
                )
            now = time.time()
            if now - last_log_time >= D2RH_TRACE_WAIT_LOG_INTERVAL:
                d2rh_trace(
                    "h2d.wait_hop1.blocked",
                    request_id=request_id,
                    tp_rank=self.tp_rank,
                    remote_request_id=remote_request_id,
                    waited_s=f"{now - hop1_start:.1f}",
                    hop1_ready=any(
                        self.staging_hop1_ready[k].is_set()
                        for k in (request_id, remote_request_id)
                        if k in self.staging_hop1_ready
                    ),
                )
                last_log_time = now
            time.sleep(0.01)
    ...
    def _transfer_kv_cache_h2d(self, req_meta: dict[str, Any]) -> None:
        ...
        if ret < 0:
            d2rh_trace(
                ...
                remote_request_id=remote_request_id,
                ret=ret,
                elapsed_s=f"{time.time() - h2d_xfer_begin:.3f}",
            )
...
class MooncakeConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str, kv_cache_config: KVCacheConfig):
        ...
        # EngineCore->Worker ZMQ START_PULL is unreliable in some deployments; default
        # to worker-local staging in start_load_kv (set True to restore scheduler path).
        self._d2rh_scheduler_start_pull = bool(
            decode_parallel_config.get("d2rh_scheduler_start_pull", False)
        )
        ...
        if self.kv_role == "kv_consumer" and self._use_d2rh:
            ...
            scheduler_ready_port = get_scheduler_ready_zmq_port(vllm_config)
            logger.info(
                "[D2RH-DEBUG] scheduler init pid=%s dp_rank=%s local_host=%s decode_tp_size=%d "
                "use_d2rh=True scheduler_ready_port=%d tp0_port_info=%s",
                os.getpid(),
                vllm_config.parallel_config.data_parallel_rank,
                self.local_host,
                self._decode_tp_size,
                scheduler_ready_port,
                format_d2rh_port_info(vllm_config, tp_rank=0),
            )
            d2rh_trace(
                "scheduler.init",
                pid=os.getpid(),
                dp_rank=vllm_config.parallel_config.data_parallel_rank,
                scheduler_start_pull=self._d2rh_scheduler_start_pull,
    ...
    def _send_start_pull(
        ...
        sock: zmq.Socket | None = None  # type: ignore
        path = make_zmq_path("tcp", self.local_host, d2rh_port)
        d2rh_trace(
            "scheduler.zmq_start_pull.send",
            request_id=request_id,
            pid=os.getpid(),
            path=path,
            decode_tp_rank=decode_tp_rank,
            timeout_s=D2RH_START_PULL_ZMQ_TIMEOUT,
        )
        logger.info(
            "[D2RH-DEBUG] START_PULL sending pid=%s path=%s request_id=%s decode_tp_rank=%d "
            "timeout=%.1fs %s",
            os.getpid(),
            path,
            request_id,
            decode_tp_rank,
            D2RH_START_PULL_ZMQ_TIMEOUT,
            format_d2rh_port_info(self.vllm_config, tp_rank=decode_tp_rank),
        )
        send_start = time.time()
        try:
            ctx = zmq.Context()  # type: ignore
            sock = make_zmq_socket(ctx=ctx, path=path, socket_type=zmq.REQ, bind=False)  # type: ignore
            sock.setsockopt(zmq.SNDTIMEO, int(D2RH_START_PULL_ZMQ_TIMEOUT * 1000))  # type: ignore
            poller = zmq.Poller()  # type: ignore
            poller.register(sock, zmq.POLLIN)  # type: ignore
            data_bytes = self.encoder.encode((START_PULL, request_id, params))
            ensure_zmq_send(sock, data_bytes, path)
            logger.info(
                "[D2RH-DEBUG] START_PULL sent waiting reply pid=%s path=%s request_id=%s elapsed=%.3fs",
                os.getpid(),
                path,
                request_id,
            ...
            resp = ensure_zmq_recv(
                ...
                poller,
                path,
                timeout=D2RH_START_PULL_ZMQ_TIMEOUT,
            )
            logger.info(
                "[D2RH-DEBUG] START_PULL ok pid=%s host=%s port=%d request_id=%s resp=%s elapsed=%.3fs",
                os.getpid(),
                self.local_host,
                d2rh_port,
                request_id,
            ...
            d2rh_trace(
                "scheduler.zmq_start_pull.ok",
                request_id=request_id,
                pid=os.getpid(),
                resp=resp.decode("utf-8", errors="replace"),
                elapsed_s=f"{time.time() - send_start:.3f}",
            )
            return resp
        except Exception as e:
            d2rh_trace(
                "scheduler.zmq_start_pull.failed",
                request_id=request_id,
                pid=os.getpid(),
                path=path,
                elapsed_s=f"{time.time() - send_start:.3f}",
            ...
            logger.error(
                "[D2RH-DEBUG] START_PULL failed pid=%s path=%s request_id=%s elapsed=%.3fs err=%s",
                os.getpid(),
                path,
                request_id,
                time.time() - send_start,
        ...
        if params is not None and params.get("do_remote_prefill"):
            ...
            if self._use_d2rh and self.kv_role == "kv_consumer":
                if request.request_id not in self.all_requests:
                    if not self._d2rh_scheduler_start_pull:
                        logger.info(
                            "[D2RH-DEBUG] scheduler defer START_PULL to worker request_id=%s "
                            "remote_request_id=%s count=%d",
                            request.request_id,
                            params.get("remote_request_id", request.request_id),
                            count,
                        )
                        d2rh_trace(
                            "scheduler.match_tokens.defer_worker",
                        ...
                        try:
                            ...
                            for decode_tp_rank in range(self._decode_tp_size):
                                ...
                                logger.info(
                                    "[D2RH-DEBUG] scheduler START_PULL target decode_tp_rank=%d %s",
                                    decode_tp_rank,
                                    format_d2rh_port_info(self.vllm_config, tp_rank=decode_tp_rank),
                                )
                                resp = self._send_start_pull(
                                    request.request_id, pull_params, d2rh_port, decode_tp_rank
                                )
                                logger.info(
                            ...
                            d2rh_trace(
                                "scheduler.match_tokens.zmq_failed",
                                request_id=request.request_id,
                                err=str(e),
                            )
                            if self.listeningthread is not None:
                    ...
                    d2rh_trace(
                        "scheduler.match_tokens.skip_inflight",
                        request_id=request.request_id,
                        hint="already in all_requests, waiting async KV load",
                    )
                    return 0, False

            d2rh_trace(
                "scheduler.match_tokens.async",
                request_id=request.request_id,
                count=count,
                num_computed_tokens=num_computed_tokens,
            )
            ...
            if params.get("remote_block_ids"):
                if all(p in params for p in ("remote_engine_id", "remote_host", "remote_port", "remote_request_id")):
                    if num_external_tokens <= 0:
                        logger.info(
                            "[D2RH-DEBUG] skip _reqs_need_recv request_id=%s num_external_tokens=%d "
                            "(START_PULL likely failed or no remote tokens)",
                            request.request_id,
                            num_external_tokens,
                        )
                        d2rh_trace(
                            "scheduler.after_alloc.skip_recv",
                            request_id=request.request_id,
                            num_external_tokens=num_external_tokens,
                            reason="num_external_tokens<=0, worker will NOT load remote KV",
                        )
                    else:
                        local_block_ids = blocks.get_unhashed_block_ids_all_groups()
                        num_local_blocks = sum(len(g) for g in local_block_ids)
                        ...
                        d2rh_trace(
                            "scheduler.after_alloc.add_recv",
                            request_id=request.request_id,
                            num_external_tokens=num_external_tokens,
                            num_local_blocks=num_local_blocks,
                            remote_request_id=params.get("remote_request_id", request.request_id),
    ...
    def build_connector_meta(
        ...
        if self._use_d2rh and (meta.requests or meta.reqs_in_batch):
            d2rh_trace(
                "scheduler.build_meta",
                n_recv=len(meta.requests),
                recv_ids=list(meta.requests.keys()),
                n_batch=len(meta.reqs_in_batch),
                batch_ids=list(meta.reqs_in_batch),
            )
            if meta.reqs_in_batch and not meta.requests:
                d2rh_trace(
                    "scheduler.build_meta.warn_no_recv",
                    batch_ids=list(meta.reqs_in_batch),
                    hint="reqs_in_batch set but _reqs_need_recv empty; check after_alloc",
                )
...
class MooncakeConnectorWorker:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str, kv_cache_config: KVCacheConfig):
        ...
        self.staging_hop1_ready: dict[str, threading.Event] = {}
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self.kv_role == "kv_producer":
            ...
            if self._use_d2rh and self.host_staging is not None:
                ...
                logger.info(
                    "[D2RH-DEBUG] worker D2RH thread init pid=%s tp_rank=%s pp_rank=%s pcp_rank=%s "
                    "d2rh_port=%s scheduler_would_use_port=%s port_match=%s host=%s worker=%s scheduler=%s",
                    os.getpid(),
                    self.tp_rank,
                    self.pp_rank,
                    self.pcp_rank,
                    ...
                    self.side_channel_host,
                    format_d2rh_port_info(
                        self.vllm_config, tp_rank=self.tp_rank, pp_rank=self.pp_rank, pcp_rank=self.pcp_rank
                    ),
                    format_d2rh_port_info(self.vllm_config, tp_rank=self.tp_rank),
                )
                self.d2rh_thread = D2RHThread(
                    ...
                    staging_hop1_ready=self.staging_hop1_ready,
    ...
    def _build_local_d2rh_pull_params(self, req_id: str, meta: ReqMeta) -> dict[str, Any]:
        prefill_tp_size = meta.remote_ptp_size if meta.remote_ptp_size is not None else self._prefill_tp_size
        remote_ranks = self._get_remote_rank(meta.remote_request_id, prefill_tp_size)
        remote_handshake_ports = [meta.remote_port + rank for rank in remote_ranks]
        remote_host, remote_engine_id = resolve_remote_host_for_handshake_port(
            meta.remote_port,
            remote_handshake_ports[0],
            meta.remote_host,
        ...
        return {
            ...
            "remote_multi_nodes_meta_mapping": meta.remote_multi_nodes_meta_mapping,
            "num_computed_tokens": meta.num_computed_tokens,
        }

    def _ensure_d2rh_staging(self, req_id: str, meta: ReqMeta) -> bool:
        if self.d2rh_thread is None or self.host_staging is None:
            return True
        if meta.remote_request_id in self.staging_allocations or req_id in self.staging_allocations:
            return True
        pull_params = self._build_local_d2rh_pull_params(req_id, meta)
        ok = self.d2rh_thread.prepare_staging_local(req_id, pull_params)
        logger.info(
            "[D2RH-DEBUG] worker local staging ensure tp_rank=%s pid=%s request_id=%s ok=%s keys=%s",
            self.tp_rank,
            os.getpid(),
            req_id,
            ok,
    ...
    def start_load_kv(self, metadata: MooncakeConnectorMetadata):
        ...
        d2rh_trace(
            "worker.start_load_kv.enter",
            tp_rank=self.tp_rank,
            pid=os.getpid(),
            n_requests=len(metadata.requests),
            request_ids=list(metadata.requests.keys()),
            n_batch=len(metadata.reqs_in_batch),
            batch_ids=list(metadata.reqs_in_batch),
            d2rh_thread_alive=self.d2rh_thread.is_alive() if self.d2rh_thread is not None else None,
        )
        ...
        for req_id, meta in metadata.requests.items():
            ...
            num_local_blocks = sum(len(group_blocks) for group_blocks in meta.local_block_ids)
            if self.host_staging is not None and (meta.num_external_tokens <= 0 or num_local_blocks == 0):
                logger.info(
                    "[D2RH-DEBUG] skip start_load_kv tp_rank=%s request_id=%s "
                    "num_external_tokens=%d num_local_blocks=%d",
                    self.tp_rank,
                    req_id,
                ...
                d2rh_trace(
                    "worker.start_load_kv.skip",
                    request_id=req_id,
                    tp_rank=self.tp_rank,
                    num_external_tokens=meta.num_external_tokens,
                    num_local_blocks=num_local_blocks,
                    reason="no external tokens or no local blocks",
                )
                continue

            if self.host_staging is not None and not self._ensure_d2rh_staging(req_id, meta):
                logger.error(
                    "[D2RH-DEBUG] skip start_load_kv: local staging ensure failed tp_rank=%s request_id=%s",
                    self.tp_rank,
                    req_id,
                )
                d2rh_trace(
                    ...
                    request_id=req_id,
                    tp_rank=self.tp_rank,
                    reason="local staging ensure failed",
                )
                continue
            ...
            for pcp_dcp_rank, remote_ports in enumerate(remote_handshake_port_list):
                for remote_tp_offset, remote_handshake_port in enumerate(remote_ports):
                    ...
                    d2rh_trace(
                        "worker.start_load_kv.queue_h2d",
                        request_id=req_id,
                        tp_rank=self.tp_rank,
...
def format_d2rh_port_info(
    ...
    dp_rank_local = getattr(parallel_config, "data_parallel_rank_local", dp_rank)
    ...
    port = D2RH_ZMQ_PORT_BASE + dp_offset + device_index
    return (
        f"port={port} base={D2RH_ZMQ_PORT_BASE} dp_rank={dp_rank} dp_rank_local={dp_rank_local} "
        f"dp_offset={dp_offset} tp_rank={tp_rank} pp_rank={pp_rank} pcp_rank={pcp_rank} "
        f"device_index={device_index} tp={tp_size} pp={pp_size} pcp={pcp_size}"
    )
