...
# request_id -> {remote_block_id: cpu_staging_block_id}
BLOCK_DICT: dict[str, dict[int, int]] = {}
# Host-level CPU staging resources (owned by tp_rank 0 worker on decode node).
_HOST_CPU_PTRS: list[int] | None = None
_HOST_CPU_KVCACHE_MANAGER: "cpu_kvcache_manager | None" = None
_CPU_STAGING_TE_RPC_PORT: int | None = None
        ...
        while True:
            try:
                ...
                if msg[0] == READY_SCHEDULER:
                    ...
                    request_id = msg[1]
                    logger.info(
                        "CPU staging full for request %s, allow scheduler to retry START_PULL",
                        request_id,
                    )
                    with self.ready_lock:
...
class D2RHThread(threading.Thread):
            ...
            try:
                ...
                if msg[0] == START_PULL:
                    ...
                    if cpu_block_ids is not None:
                        BLOCK_DICT[request_id] = dict(zip(remote_block_ids, cpu_block_ids))
                    logger.info(
                    ...
                    if cpu_block_ids is not None:
                        ...
                        self.send_staging_full(request_id)
                        pull_ack = STAGING_FULL

                    # Acknowledge the START_PULL request.
                    while True:
                        try:
                            sock.send_multipart((identity, b"", pull_ack), flags=zmq.NOBLOCK)  # type: ignore
                            break
                        except zmq.Again:  # type: ignore
                            logger.debug("Socket not ready, retrying to send START_PULL resp for %s", msg[1])
                            time.sleep(0.01)
                else:
                    logger.error("Connection listener got unexpected message %s", msg)
            except Exception as e:
    ...
    def _transfer_kv_cache(self, req_meta: dict[str, Any]):
        ...
        if ret < 0:
            ...
            # Notify prefill that KV has been pulled into CPU staging and P blocks can be freed.
    ...
    def send_staging_full(self, request_id: str) -> None:
        """Tell scheduler that CPU staging is full so it can retry START_PULL later."""
        sock: zmq.Socket | None = None  # type: ignore
        try:
            port = 8200
            sock = self._get_remote_socket(self.host_ip, port)
            data_bytes = self.encoder.encode((STAGING_FULL, request_id))
            ...
            logger.debug("STAGING_FULL response for request %s: %s", request_id, resp.decode("utf-8"))
            if resp != b"ACK":
                logger.error(
                    "Failed to receive ACK for STAGING_FULL request %s from %s:%d",
                    request_id,
                    self.host_ip,
                    port,
                )
            ...
            if isinstance(sock, zmq.Socket):  # type: ignore
                ...
                sock = None
                logger.warning(
                    "Unexpected error occurred in STAGING_FULL socket, %s, closing the original channel",
                    e,
                )
        finally:
            if sock is not None:
    ...
    def _send_done_recv_signal(
        ...
        """Tell prefill node that KV has been received into CPU staging."""
        logger.debug(
        ...
        try:
            ...
            # Empty remote_port_send_num: one consolidated D2RH pull replaces per-rank D2D pulls.
...
class KVCacheRecvingThread(threading.Thread):
    ...
    def _transfer_kv_cache(self, req_meta: dict[str, Any]):
        ...
        local_host = get_ip()
        cpu_block_map = BLOCK_DICT.get(remote_request_id) or BLOCK_DICT.get(req_meta["request_id"])
        ...
        if req_meta.get("all_task_done") and self.cpu_kvcache_manager is not None:
            ...
            BLOCK_DICT.pop(remote_request_id, None)
            BLOCK_DICT.pop(req_meta["request_id"], None)
        if ret < 0:
...
class MooncakeConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        if self.kv_role == "kv_consumer":
            ...
            self.listeningthread = HostListeningThread(self.all_requests)
    ...
    def get_num_new_matched_tokens(
        ...
        if params is not None and params.get("do_remote_prefill"):
            if request.request_id not in self.all_requests:
                sock: zmq.Socket | None = None  # type: ignore
                try:
                    sock = self._get_remote_socket(self.local_host,
                                                   self.local_port)
                    data_bytes = self.encoder.encode(
                        (START_PULL, request.request_id, params))
                    ...
                    logger.debug(
                        f"Received response for request {request.request_id}: {resp.decode('utf-8')}"
                    )
                    ...
                    if resp != b"ACK":
                        logger.error(
                            ...
                            request.request_id, self.local_host,
                            self.local_port)
                        raise RuntimeError(
                    ...
                    if sock is not None:
                        self._return_remote_socket(sock, self.local_host,
                                                   self.local_port)
...
class MooncakeConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self.kv_role == "kv_consumer":
            global _HOST_CPU_PTRS, _HOST_CPU_KVCACHE_MANAGER, _CPU_STAGING_TE_RPC_PORT
            ...
            if self.tp_rank == 0:
                ...
                _HOST_CPU_PTRS = cpu_kv_caches_base_addr
                _HOST_CPU_KVCACHE_MANAGER = cpu_kvcache_manager_instance
                _CPU_STAGING_TE_RPC_PORT = self.te_rpc_port
                os.environ["VLLM_MOONCAKE_CPU_STAGING_RPC_PORT"] = str(self.te_rpc_port)
                os.environ["VLLM_MOONCAKE_CPU_STAGING_PTRS"] = ",".join(
                    str(p) for p in cpu_kv_caches_base_addr
                )
                ...
                staging_port = os.environ.get("VLLM_MOONCAKE_CPU_STAGING_RPC_PORT")
                staging_ptrs = os.environ.get("VLLM_MOONCAKE_CPU_STAGING_PTRS")
                if staging_port and staging_ptrs:
                    cpu_te_rpc_port = int(staging_port)
                    cpu_kv_caches_base_addr = [int(p) for p in staging_ptrs.split(",") if p]
                else:
                    logger.warning(
                        "CPU staging env not found on tp_rank %d; "
                        "decode KV pull may fail unless tp_size=1.",
                        self.tp_rank,
                    )
                cpu_kvcache_manager_instance = None
