...
CANCEL_PULL = b"cancel_pull"
# Backoff before retrying START_PULL after STAGING_FULL (seconds).
STAGING_FULL_RETRY_BACKOFF_S = 0.1

# ZMQ ports for D2RH (hop1) and scheduler ready signaling (hop1 done).
# Offset layout matches side_channel_port + device_index:
...
class cpu_kvcache_manager():
    ...
    def alloc_blocks(self, req_need_blocks):
        ...
        if req_need_blocks > len(self.free_queue):
            logger.debug(
                "CPU staging short: need=%d free=%d",
                req_need_blocks,
                now_block,
            )
            return None
    ...
    def free_blocks(self, req_block_ids):
        if not req_block_ids:
            return
        if isinstance(req_block_ids[0], int):
            flat_block_ids = list(req_block_ids)
        else:
            flat_block_ids = [block_id for sublist in req_block_ids for block_id in sublist]
        logger.info(
            "[===] free CPU staging blocks %s (free=%d)",
            flat_block_ids,
            len(self.free_queue) + len(flat_block_ids),
        )
...
class D2RHThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            path = make_zmq_path("tcp", self.host_ip, handshake_port)
            logger.info("Starting D2RH listener on path: %s", path)
    ...
    def run_busy_loop(self, sock: zmq.Socket):  # type: ignore
        ...
        while True:
            try:
                ...
                msg = decoder.decode(payload[0])
                logger.debug("[===] D2RH received msg[0]=%s", msg[0])
                if msg[0] == CANCEL_PULL:
                    request_id = msg[1]
                    self._cancel_staging(request_id)
                    while True:
                        try:
                            sock.send_multipart((identity, b"", b"ACK"), flags=zmq.NOBLOCK)  # type: ignore
                            break
    ...
    def _cancel_staging(self, request_id: str) -> None:
        with self._cancel_lock:
            self.cancelled_requests.add(request_id)
            block_map = self.remote_local_block_map.pop(request_id, None)
            if block_map:
                self.cpu_kvcache_manager.free_blocks(list(block_map.values()))

    def _handle_request(self, req_meta: dict[str, Any]):
        request_id = req_meta["request_id"]
        with self._cancel_lock:
            if request_id in self.cancelled_requests:
                cpu_block_ids = req_meta.get("grouped_local_block_ids") or []
                if cpu_block_ids and isinstance(cpu_block_ids[0], list):
                    cpu_block_ids = [bid for group in cpu_block_ids for bid in group]
                if cpu_block_ids:
                    self.cpu_kvcache_manager.free_blocks(cpu_block_ids)
    ...
    def _transfer_kv_cache(self, req_meta: dict[str, Any]):
        ...
        remote_handshake_ports = req_meta["remote_handshake_ports"]
        ...
        with self._cancel_lock:
            if transfer_ok and request_id not in self.cancelled_requests:
                self.send_pull_done(request_id)
            elif request_id in self.cancelled_requests:
                block_map = self.remote_local_block_map.pop(request_id, None)
                if block_map:
                    self.cpu_kvcache_manager.free_blocks(list(block_map.values()))
...
class MooncakeConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        self.scheduler_ready_port = get_scheduler_ready_zmq_port(vllm_config)

        if self.kv_role == "kv_consumer":
            self.all_requests: set[str] = set()
            self._staging_backoff_until: dict[str, float] = {}
            self.listeningthread = HostListeningThread(
                self.all_requests,
                self._decode_tp_size,
                self.scheduler_ready_port,
            )
    ...
    def _send_cancel_pull(self, request_id: str, d2rh_port: int) -> None:
        sock: zmq.Socket | None = None  # type: ignore
        try:
            sock = self._get_remote_socket(self.local_host, d2rh_port)
            data_bytes = self.encoder.encode((CANCEL_PULL, request_id))
            ...
            if resp != b"ACK":
                logger.warning(
                    "CANCEL_PULL for request %s on port %s got %s",
                    request_id,
                    d2rh_port,
                    resp,
                )
            ...
            logger.warning(
                "CANCEL_PULL failed for request %s on port %s: %s",
                request_id,
                d2rh_port,
                e,
            )
    ...
    def get_num_new_matched_tokens(
        ...
        if params is not None and params.get("do_remote_prefill"):
            if request.request_id not in self.all_requests:
                backoff_until = self._staging_backoff_until.get(request.request_id, 0.0)
                if time.monotonic() < backoff_until:
                    return None, False
                acked_ranks: list[int] = []
                try:
                    ...
                    for decode_tp_rank in range(self._decode_tp_size):
                        ...
                        logger.debug(
                            "START_PULL response for request %s decode_tp_rank=%s port=%s: %s",
                            request.request_id,
                            decode_tp_rank,
                            d2rh_port,
                            resp.decode("utf-8"),
                        ...
                        acked_ranks.append(decode_tp_rank)
                    if got_staging_full:
                        for decode_tp_rank in acked_ranks:
                            d2rh_port = get_d2rh_zmq_port(self.vllm_config, decode_tp_rank)
                            self._send_cancel_pull(request.request_id, d2rh_port)
                        ...
                        self._staging_backoff_until[request.request_id] = (
                            time.monotonic() + STAGING_FULL_RETRY_BACKOFF_S
                        )
                        return None, False
                    self._staging_backoff_until.pop(request.request_id, None)
