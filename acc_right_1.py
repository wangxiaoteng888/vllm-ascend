...
class KVCacheTaskTracker:
    ...
    def update_done_task_count(self, request_id: str):
        logger.info(f"[===] free_request {request_id=}")
        with self.done_task_lock:
        ...
        while True:
            try:
                ...
                if msg[0] == READY_SCHEDULER:
                    logger.info("Got READY_SCHEDULER for request %s", msg[1])
                    request_id = msg[1]
...
class D2RHThread(threading.Thread):
            ...
            try:
                ...
                if msg[0] == START_PULL:
                    ...
                    if cpu_block_ids is not None:
                        ...
                        logger.error(f"Have not enough tokens")
                    logger.info(
    ...
    def _send_done_recv_signal(
        ...
        try:
            ...
            logger.info("D2RH received response for request %s: %s", request_id, resp.decode("utf-8"))
            if resp != b"ACK":
                logger.error(
    ...
    def _handle_request(self, req_meta: dict[str, Any]):
        ...
        try:
            ...
            if not self.use_cpu_staging:
                ...
                logger.error(f"[===] we get some error")
