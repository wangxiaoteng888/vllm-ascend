...
class KVCacheSendingLayerThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            #     )
            # else:
            #     logger.info(f"[HB] checking peer connectivity before transfer: {host}:{port} (req={req_id})")
            #     alive = False
            ...
            #             logger.warning(f"[HB] is_peer_alive raised on {host}:{port}, attempt={attempt+1}: {_hb_e}")
            #         time.sleep(0.2 * (2 ** attempt))  # backoff: 0.2s, 0.4s, 0.8s

            #     logger.info(f"[HB] peer {host}:{port} alive={alive} (req={req_id})")
            #     if not alive:
            #         # 这里选择“丢弃本次发送并继续循环”；如果你更想延后重试，也可以改成 requeue：
            #         # self.send_queue.put((req_id, req_meta, layer_index, key, value))
            #         logger.error(f"[HB] peer not alive, skip transfer for req={req_id}, layer={layer_index}")
            #         continue
...
class HeartbeatMonitor(threading.Thread):
    ...
    def add_target(self, host: str, port: int):
        ...
        with self._targets_lock:
            ...
            logger.debug(f"[HB] counters init for {host}:{port} -> {self._counters[key]}")
    ...
    def is_alive(self, host: str, port: int) -> bool:
        ...
        return (time.time() - last_ok) < (3 * self.poll_interval + self.timeout)
            ...
            for key, sock in items:
                try:
                    ...
                    try:
                        ...
                        self._counters[key] = {"sent": 1, "recv": 0, "timeout": 0}
                    host, port = key
                    logger.debug(f"[HB->] ping {host}:{port} sent={self._counters[key]['sent']}")
                except Exception:
                ...
                if sock in events:
                    try:
                        ...
                        if reply == HEARTBEAT_ACK:
                            with self._targets_lock:
                                self._alive[key] = now
                            self._counters[key]["recv"] = self._counters.get(key, {}).get("recv", 0) + 1
                    except Exception:
                        # 没回/异常都不用紧，下一轮再查
                        pass
            # 间隔几分钟再查一次
...
class MooncakeLayerwiseConnectorWorker:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        self._watch_thread = threading.Thread(
            target=self._peer_watch_loop, name="PeerWatcher", daemon=True
        )
        self._watch_thread.start()
        logger.info(f"[WATCH] Peer watcher started, interval={self._watch_interval}s")

    def _register_peer(self, eng: Optional[str], host: Optional[str], port: Optional[int]) -> None:
        ...
        try:
            ...
            logger.warning(f"[WATCH] add_target failed for {host}:{port}: {_e}")
        else:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self.vllm_config.kv_transfer_config.is_kv_producer:
            ...
            self.kv_send_layer_thread = KVCacheSendingLayerThread(
                ...
                is_peer_alive=lambda host, port: self.heartbeat.is_alive(host, port))
            ...
            try:
                ...
                for eng, (host, port) in snapshot:
                    try:
                        ...
                        logger.warning(f"[WATCH] is_alive exception for {host}:{port}: {_e}")
                        alive = False
                    logger.info(f"[WATCH] engine={eng} peer {host}:{port} alive={alive}")

                    if not alive:
                        # 仅清理该 peer 的元数据条目（engine_id + port）
                        try:
                            with self._peer_meta_lock:
                                self.remote_te_port.get(eng, {}).pop(port, None)
                                self.remote_kv_caches_base_addr.get(eng, {}).pop(port, None)
                            logger.warning(f"[WATCH] purged meta for down peer: engine={eng} {host}:{port}")
                        except Exception as _e:
                            logger.warning(f"[WATCH] purge meta error for {host}:{port}: {_e}")
                        # 同时从心跳监控中移除该目标（会注销 poller 并关闭 socket）
                        try:
                            self.heartbeat.remove_target(host, port)
                        except Exception as _e:
                            logger.warning(f"[WATCH] heartbeat.remove_target failed for {host}:{port}: {_e}")
