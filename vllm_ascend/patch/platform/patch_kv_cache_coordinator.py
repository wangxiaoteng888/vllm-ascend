...
def _install_d2rh_manager_bridge() -> None:
    """Enable per-group HBM hits for DeepSeek-V4 D2RH consumers.

    The upstream manager only selects independent per-group lookup for Mamba
    hybrid caches. DeepSeek-V4 combines compressed MLA with sliding-window MLA,
    so an all-group lookup can collapse a valid local HBM hit to zero.
    """
    ...
    # so patch import order cannot leave the upstream factory cached.
    ...
    def get_computed_blocks(self, request):
        ...
        # Independent group lookup has no sparse junction to pin.
        return (
